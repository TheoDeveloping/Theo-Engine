...
class PlayState extends MusicBeatState
{
	...
	public static var stageAdd:Stages;
	...
	public static var inmortalTxt:FlxText;
	...
	var dialogue:Array<String> = [':void:You forgot add the dialogue txt'];

	public static var hasDialog:Bool;
	...
	override public function create()
	{
		...
		if (OpenFlAssets.exists(Paths.dialogueTxt('dialogue/' + curSong + '/dialogue')) && !hasDialog)
			hasDialog = true;
		...
		DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")" + " | Score : " + scoreCounter + " | Misses : " + missesCounter, iconRPC);
		#end

		//stage shit
		stageAdd = new Stages(curStage);
		add(stageAdd);
		...
		timeTxt = new FlxText((FlxG.width / 2) - 206, 19, 400, SONG.song, 32);
		timeTxt.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		timeTxt.scrollFactor.set();
		timeTxt.borderSize = 2;
		
		//if(downscroll_isenabled) timeTxt.y = FlxG.height - 44;
		...
		timeBar.createFilledBar(0xFF9E9E9E, 0xFF66FF33);
		...
		var defaultCamZoom = stageAdd.defaultCamZoom;
		...
		scoreBar.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, OUTLINE, FlxColor.BLACK);
		scoreBar.scrollFactor.set();
		add(scoreBar);

		inmortalTxt = new FlxText(healthBarBG.x + healthBarBG.width - 190, healthBarBG.y + 30, 0, "", 20);
		inmortalTxt.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, OUTLINE, FlxColor.BLACK);
		inmortalTxt.scrollFactor.set();
		inmortalTxt.size = 15;
		inmortalTxt.y -= 200;
		add(inmortalTxt);

		aboutBar = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Normal" : "Easy") + " - Theo Engine " + "v." + Application.current.meta.get('version'),  16);
		aboutBar.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		aboutBar.scrollFactor.set();
		add(aboutBar);
		...
		if (hasDialog) {
			trace(curSong + ' has dialogue, has a path');
		}
	}
	...
	function startSong():Void
	{
		...
		DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")" + " | Score : " + scoreCounter + " | Misses : " + missesCounter, iconRPC, true, songLength);
		#end
	}
	...
	function updateAccuracy():Void
		{
			totalPlayed += 1;
			accuracy = totalNotesHit / totalPlayed * 1;
		}
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			if (startTimer.finished)
			{
				DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")" + " | Score : " + scoreCounter + " | Misses : " + missesCounter, iconRPC,  true, songLength - Conductor.songPosition);
			}
			else
			{
				DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")" + " | Score : " + scoreCounter + " | Misses : " + missesCounter, iconRPC);
	...
	function truncateFloat( number : Float, precision : Int): Float 
		{
			var num = number;
			num = num * Math.pow(1, precision);
			num = Math.round( num ) / Math.pow(1, precision);
			return num;
		}
	...
	override public function update(elapsed:Float)
	{
		...
		scoreBar.text = "Score : " + scoreCounter + " | Misses : " + missesCounter + " | Accuracy : " + truncateFloat(accuracy*1, 2) + "%" + " | Rating : " + rating;
		scoreBar.screenCenter(X);

		if (FlxG.save.data.inmortalMode)
			{
				inmortalTxt.text = "INMORTAL MODE";
			}
		else
			{
				inmortalTxt.text = "";
			}
		inmortalTxt.screenCenter(X);
		...
		if (!inCutscene && !_exiting)
		{
			...
			if (health <= 0 && !FlxG.save.data.inmortalMode)
			{
				...
				DiscordClient.changePresence("Game Over In " + detailsText, SONG.song + " (" + storyDifficultyText + ")", iconRPC);
				#end
			}
		}
	...
	function endSong():Void
	{
		...
		{
			#if !switch
			Highscore.saveScore(SONG.song, scoreCounter, storyDifficulty);
			#end
		}
