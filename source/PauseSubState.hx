...
class PauseSubState extends MusicBeatSubstate
{
	...
	var pauseOG:Array<String> = ['Resume', 'Restart Song', 'Change Difficulty', 'Inmortal Mode', 'Exit to menu'];
	...
	public function new(x:Float, y:Float)
	{
		...
		inmortalTxt = new FlxText(20, 15 + 96, 0, "Inmortal Mode: True", 32);
		inmortalTxt.scrollFactor.set();
		inmortalTxt.setFormat(Paths.font('vcr.ttf'), 32);
		inmortalTxt.updateHitbox();
		inmortalTxt.x = FlxG.width - (inmortalTxt.width + 20);
		inmortalTxt.visible = PlayState.inmortalTxt.visible;
		add(inmortalTxt);
			...
			switch (daSelected)
			{
				...
				case "Inmortal Mode":
					if (!FlxG.save.data.inmortalMode)
						FlxG.save.data.inmortalMode = true;
					else
						FlxG.save.data.inmortalMode = false;

					inmortalTxt.visible = !FlxG.save.data.inmortalMode;
